...
NUM_MODELS = 3
T = 20  # number of passes per model

USE_AUGMENTED = True   # use augmented ensemble
USE_TEST = True        # True = ISIC test, False = validation

model_tag = "aug" if USE_AUGMENTED else "baseline"
dataset_csv = "splits/isic_test.csv" if USE_TEST else "splits/val.csv"

output_file = f"results/mcd_ensemble_predictions_{model_tag}_{'test' if USE_TEST else 'val'}.csv"
...
for i in range(NUM_MODELS):
    model = build_model()
    model.load_state_dict(torch.load(f"models/ensemble/model_{model_tag}_{i}.pth"))
    model.to(device)
...
print(f"Saved MCD+Ensemble results to {output_file}")
